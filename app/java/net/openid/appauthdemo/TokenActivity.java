...
//        }
    ...
    protected void onStart() {
        ...
        if (response != null && response.authorizationCode != null) {
            ...
            Log.w("Authorization flow failed:",""+ex.getMessage());

        } else {

            displayNotAuthorized("No authorization state retained - reauthorization required"+ex.getMessage());
...
//        }
    ...
    private void displayAuthorized() {
        ...
        Log.e("refresh_token","refresh_token : "+((state.getRefreshToken() == null)
                ? "no_refresh_token_returned"
                : "refresh_token_returned : "+state.getRefreshToken()));

        Log.e("IdToken","IdToken : "+(state.getIdToken()) == null
                ? "no_id_token_returned"
                : "IdToken : "+state.getIdToken());

        if (state.getAccessToken() == null) {
            Log.e("AccessToken","AccessToken : no_access_token_returned");
        } else {
            Long expiresAt = state.getAccessTokenExpirationTime();
            if (expiresAt == null) {
                Log.e("AccessToken","AccessToken : no_access_token_expiry");
            } else if (expiresAt < System.currentTimeMillis()) {
                refreshAccessToken();
                Log.e("AccessToken","AccessToken : access_token_expired");
            } else {
                String template = getResources().getString(R.string.access_token_expires_at);
                Log.e("AccessToken","AccessToken : "+String.format(template,
                        DateTimeFormat.forPattern("yyyy-MM-dd HH:mm:ss ZZ").print(expiresAt)));
            }
        }

        String token = state.getIdToken();
        ...
        String Data = "";
        Iterator myVeryOwnIterator = jwt.getClaims().keySet().iterator();
        while(myVeryOwnIterator.hasNext()) {
            String key=(String)myVeryOwnIterator.next();
            Claim value= jwt.getClaims().get(key);

            if(key.equals("name")){
                Data =  "Name: "+value.asString();
            }

        }
        ((TextView)findViewById(R.id.userinfo_name)).setText(Data);
    ...
    private void handleCodeExchangeResponse(
        ...
        if (!mStateManager.getCurrent().isAuthorized()) {
            final String message = "Authorization Code exchange failed"
                    + ((authException != null) ? authException.errorDescription : "");
